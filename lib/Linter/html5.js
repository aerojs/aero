...
module.exports = function*(method, route, response) {
	...
	if(!this.production && method === 'GET' && contentType && contentType.includes('text/html')) {
		let displayHTMLErrors = errors => {
			...
			errors = errors.filter(error => error.rule !== 'tag-name-match' && error.rule !== 'indent-style')

			if(errors.length === 0)
				return;

			console.log(`[${this.chalk.red(route)}] HTML 5 issues:`)

			errors.forEach(error => {
				let keys = Object.keys(error.data)
				let prefix = this.chalk.yellow(error.rule)
