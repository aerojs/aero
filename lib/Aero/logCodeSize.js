...
module.exports = function(name, elements, color) {
	...
	elements.forEach(element => {
		...
		console.log(color(element.id), ' '.repeat(Math.max(this.columns - 5 - size.length - element.id.length, 0)), size, chalk.dim('KB'))
	})
	...
	console.log(title, ' '.repeat(Math.max(this.columns - 5 - chalk.stripColor(title).length - size.length, 0)), color(size), chalk.dim('KB'))
}