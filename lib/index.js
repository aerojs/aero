...
aero.registerEventListeners = function() {
	...
	// Page modifications
	this.events.on('page modified', function(pagePath) {
		aero.loadPage(pagePath);
	});
...
aero.watchFiles = function() {
	...
	watch(this.config.path.pages, function(filePath) {
		aero.events.emit('page modified', path.dirname(filePath));
	});
...
// loadPage
aero.loadPage = function(pagePath) {
	let pageId = path.relative(aero.config.path.pages, pagePath);
