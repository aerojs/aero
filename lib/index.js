...
aero.registerEventListeners = function() {
	...
	this.events.on('config loaded', Promise.coroutine(function*() {
		...
		yield Promise.each(Object.keys(aero.config.plugins), function(pluginName) {
			...
			if(plugin.styles) {
				return Promise.all(plugin.styles.map(function(styleName) {
					let stylePath = require.resolve(`../plugins/${pluginName}/${styleName}.styl`);
					return loadStyle(stylePath);
				})).each(function(style) {
					aero.pluginStyles.push(style);
				});
			}
		...
		if(aero.config.security && aero.config.security.key && aero.config.security.cert) {
			...
			if(!aero.security.key)
				console.error(`Failed to load security key: ${aero.config.security.key}`);

			if(!aero.security.cert)
				console.error(`Failed to load security certificate: ${aero.config.security.cert}`);
		}
	...
	this.events.on('page loaded', function(page) {
		...
		let js = aero.liveReload.script + aero.pluginScripts.join(';') + aero.js.join(';');
