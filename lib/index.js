...
let Static = require('./classes/Static')
...
aero.registerEventListeners = function() {
		...
		.then(function() {
			...
			if(aero.pages.size === 0) {
				...
				mkdir(homePath)
					...
					.then(() => aero.loadPage('home')).then(function() {
						aero.watchFiles()
					})
			} else {
				aero.watchFiles()
	...
	this.events.on('page loaded', function(page) {
		...
		let js = aero.liveReload.script + aero.pluginScripts.join(';') + aero.js.join(';') // '"use strict";' +
		...
		if(page.controller && page.controller.render) {
			...
			if(aero.layout.controller) {
					...
					renderLayout(request, function(layoutControllerParams) {
						renderPage(request, function(params) {
							...
							if(layoutControllerParams) {
								if(aero.layout.json)
									layoutControllerParams = Object.assign(aero.layout.json, layoutControllerParams)
								...
								respond(renderLayoutTemplate(Object.assign(aero.layout.json, {
									content: code,
									css: css,
									js: js,
				...
				aero.get(page.url, function(request, response) {
					renderPage(request, function(params) {
						...
						if(aero.layout.json)
							layoutParams = Object.assign(aero.layout.json, layoutParams)
			...
			if(page.controller) {
				...
				if(aero.layout.json)
					layoutParams = Object.assign(layoutParams, aero.layout.json)
	...
	aero.events.on('route modified', function(relativePath) {
		let url = `${aero.server.protocol}://localhost:${aero.config.port}/${relativePath}`;

		if(aero.serverStarted) {
			requester.getAsync({uri: url, timeout: 2000, encoding: null}).then(function(response) {
				...
				if(aero.verbose) {
					...
					let okMessage = `${url}`
					okMessage += ' '.repeat(50 - okMessage.length)

					let lengthMessage = `${(response.body.length / 1024).toFixed(1)} KB`;
					lengthMessage = ' '.repeat(60 - okMessage.length - lengthMessage.length) + lengthMessage

					console.log(chalk.green(okMessage), chalk.dim(lengthMessage))
				}
...
aero.get = function(url, func) {
	...
	aero.events.emit('route modified', url);
}
