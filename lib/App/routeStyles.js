...
module.exports = function() {
	let combinedCSS = this.pluginStyles.join(' ') + this.css.map(style => style.code).join(' ')

	if(this.layout.css)
		combinedCSS += this.layout.css

	if(this.fontsCSS)
		combinedCSS = this.fontsCSS + combinedCSS
	...
	this.server.routes.GET['styles.css'] = (request, response) => {
		...
		if(acceptEncoding && acceptEncoding.indexOf('br') !== -1) {
			...
			response.writeHead(200, {
				'Content-Type': 'text/css',
				'Cache-Control': 'max-age=864000',
				'Content-Length': Buffer.byteLength(combinedCSS, 'utf8')
			})
			response.end(combinedCSS)
		}
	}
}