...
module.exports = function*(method, route) {
	...
	try {
		...
		if(page) {
			...
			page.codeSize = response.body.length
		}
		...
		let pageStatusLog = msg => {
			...
			let ok = `${url}`
			ok += ' '.repeat(Math.max(this.columns - 2 - ok.length - chalk.stripColor(msg).length - methodMsg.length, 0))
		...
		// HTML5 validator
		if(this.config.validation.html5 && method === 'GET' && contentType && contentType.includes('text/html')) {
