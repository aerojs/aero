...
let Page = Promise.coroutine(function*(id, pagePath, pageLoadCallBack) {
	...
	let browserScript = components.script ? `<script>var aeroOnEvent='DOMContentLoaded';var aeroBrowserScript=function(){document.removeEventListener(aeroOnEvent,aeroBrowserScript);${components.script}};if(document.readyState==='complete'){aeroBrowserScript();}else{document.addEventListener(aeroOnEvent,aeroBrowserScript);}</script>` : ''
