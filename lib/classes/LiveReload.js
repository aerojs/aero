...
class LiveReload {
	constructor(security, port) {
		...
		if(security && security.key && security.cert) {
			app = require('spdy').createServer(security, this.processRequest)
			protocol = 'wss'
		} else {
		...
		let scriptPath = require.resolve('../scripts/live-reload.js')
		this.script = UglifyJS.minify(scriptPath).code.replace('${protocol}', protocol).replace('${port}', port)
