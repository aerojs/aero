...
class Aero {
	constructor() {
		...
		aero.run = Promise.coroutine(function*() {
			...
			aero.events.on('all pages loaded', () => console.timeEnd('Startup'))
	...
	registerEventListeners() {
		...
		aero.events.on('config loaded', Promise.coroutine(function*() {
			...
			yield aero.loadPlugins.then(() => console.timeEnd('Plugins'))
			...
			aero.layout = yield new Layout(aero.config.path.layout, function(layout) {
				...
				Promise.all([
					recompileStyles().then(() => console.timeEnd('Styles')),
					recompileScripts().then(() => console.timeEnd('Scripts'))
				])
				.then(() => console.time('Pages'))
				...
				.then(() => console.timeEnd('Fonts'))
				.then(reloadPages)
				.then(() => console.timeEnd('Pages'))
