...
class Aero {
	constructor() {
		...
		aero.run = Promise.coroutine(function*() {
			let config = yield getFile('config.hjson', defaultConfig).then(contents => hjson.parse(contents + '\n'))
	...
	registerEventListeners() {
			...
			.then(function() {
				...
				if(aero.pages.size === 0) {
					...
					mkdir(homePath)
						...
						.then(() => fs.writeFileAsync(homeJSON, hjson.stringify({url: ''}, {space: '\t'}), 'utf8'))
		...
		aero.events.on('page loaded', function(page) {
				...
				if(page.template) {
					if(aero.layout.controller) {
						...
						methodNames.forEach(function(name) {
							let next = page.controller[name].bind(page.controller)
							...
							aero[method](page.url, function(request, response) {
								renderLayout(request, function(layoutControllerParams) {
									response.render = function(params) {
										let code = page.wrap(renderPageTemplate(Object.assign({}, params, request.globals)))
						...
						// ----------------------------
						methodNames.forEach(function(name) {
							let runPageController = page.controller[name].bind(page.controller)

							aero[name](page.url, function(request, response) {
								response.render = function(params) {
			...
			// Routing
			if(page.controller && page.controller.get && page.template) {
				...
				if(page.controller.post) {
					let postPage = page.controller.post.bind(page.controller)

					aero.post(page.url, function(request, response) {
						response.render = getPageRenderFunction(page, request, response)
				...
				if(aero.layout.controller) {
					...
					let getPage = page.controller.get
