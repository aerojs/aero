		...
		aero.events.on('config loaded', Promise.coroutine(function*() {
			...
			if(aero.config.security && aero.config.security.key && aero.config.security.cert) {
				...
				if(!aero.security.key)
					aero.error(chalk.red(`Failed to load security key: ${aero.config.security.key}`))

				if(!aero.security.cert)
					aero.error(chalk.red(`Failed to load security certificate: ${aero.config.security.cert}`))
			}
			...
			if(aero.serverStarted) {
				...
				requester[`${method.toLowerCase()}Async`]({uri: url, timeout: 2000, encoding: null}).then(function(response) {
					...
					if(response.statusCode === 404) {
						aero.error('Not found:', chalk.red(`${url}`))
					...
					if(method === 'GET' && contentType && contentType.includes('text/html')) {
						...
						.then(function(messages) {
							if(messages.length === 0) {
								console.log(chalk.blue('/' + relativePath), chalk.green('Valid HTML5!'))
								return
							}
					...
					aero.error(chalk.red(`Error fetching ${url}`))
