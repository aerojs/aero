	...
	registerEventListeners() {
		...
		aero.events.on('config loaded', Promise.coroutine(function*() {
			...
			if(aero.config.fonts.length > 0) {
				...
				aero.downloadFonts = download('https://fonts.googleapis.com/css?family=' + aero.config.fonts.join('|'))
			}
			...
			if(aero.serverStarted) {
				...
				requester[`${method.toLowerCase()}Async`]({uri: url, timeout: 2000, encoding: null}).then(function(response) {
					...
					if(aero.verbose) {
						...
						let ok = `${url}`
						ok += ' '.repeat(Math.max(53 - ok.length, 0))
						...
						console.log(methodMsg, chalk.blue(ok), size, time)
					}

					// HTML5 validator
					let contentType = response.headers['content-type']
					if(method === 'GET' && contentType && contentType.includes('text/html')) {
						checkHTML5Response(response)
						.then(function(messages) {
							if(messages.length === 0) {
								let validMsg = 'HTML 5'
								console.log(chalk.blue('/' + relativePath + ' '.repeat(Math.max(76 - validMsg.length - relativePath.length))), chalk.green(validMsg))
								return
							}

							messages.forEach(function(msg) {
								console.log(chalk.blue('/' + relativePath), chalk.yellow(msg.message))
							});
						})
