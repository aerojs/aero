...
class Aero {
	constructor() {
		...
		this.run = Promise.coroutine(function*() {
			...
			this.pastEvents.forEach(function(registration) {
				this.events.on(registration.eventName, registration.func)
			})

			// Register past middleware
			this.pastMiddleware.forEach(function(middleware) {
			...
			// Benchmark startup
			this.time('All')
			this.events.on('all pages loaded', function() {
				this.timeEnd('All')
				this.separator()
	...
	use() {
		for (let i = 0; i < arguments.length; i++) {
			if(this.server)
				...
				this.pastMiddleware.push(arguments[i])
		}
	}
	...
	registerEventListeners() {
		...
		let recompileStyles = () => {
			...
			let asyncStyleCompileTasks = this.config.styles.map(function(styleId) {
				if(this.verbose)
		...
		let recompileScripts = () => {
			...
			let asyncScriptCompileTasks = this.config.scripts.map(function(scriptId) {
				if(this.verbose)
		...
		this.events.on('config loaded', Promise.coroutine(function*() {
			// Favicon
			loadFavIcon(this.config.favIcon, function(imageData) {
			...
			if(this.config.security && this.config.security.key && this.config.security.cert) {
				this.security = yield Promise.props({
			...
			// Static files
			this.config.static.forEach(this.static.add)
			...
			this.loadPlugins = Promise.each(Object.keys(this.config.plugins), pluginName => {
				if(this.verbose)
					this.loading('plugin', chalk.yellow(pluginName))
				...
				if(plugin.init) {
					tasks.push(
						plugin.init(aero, this.config.plugins[pluginName])
					)
				}
			...
			this.layout = yield new Layout(this.config.path.layout, layout => {
				...
				Promise.all([
					...
					.then(() => {
						...
						this.css.forEach(function(style) {
							if(!style.code) {
				...
				if(page.template) {
					...
					if(this.layout.controller) {
						...
						page.httpVerbs.forEach(method => {
							...
							aero[method](page.url, (request, response) => {
		...
		this.events.on('route modified', function(method, url) {
			// Live reload
			if(this.liveReload) {
				this.liveReload.broadcast({
		...
		this.events.on('config modified', function() {
			if(this.verbose) {
