...
class Aero {
	constructor() {
		aero = this
		aero.verbose = true
		aero.overwriteLoadingMessages = false
		aero.showStyleSize = true
		aero.pastEvents = []
		aero.pastMiddleware = []
		...
		aero.run = Promise.coroutine(function*() {
			...
			aero.pastEvents.forEach(function(registration) {
				aero.events.on(registration.eventName, registration.func)
			})

			// Register past middleware
			aero.pastMiddleware.forEach(function(middleware) {
				aero.server.modifiers.push(middleware)
			})
			...
			// Benchmark startup
			aero.time('All')
			aero.events.on('all pages loaded', function() {
				aero.timeEnd('All')
				aero.separator()

				// Log benchmark times
				if(aero.verbose) {
					Object.keys(aero.benchmarks)
						.forEach(phase => console.log(phase + ':', ' '.repeat(Math.max(14 - phase.length - aero.benchmarks[phase].toString().length, 0)), aero.benchmarks[phase] + chalk.dim(' ms')))
	...
	unroute(pageId) {
		...
		Object.keys(aero.server.routes).forEach(function(method) {
			let routes = aero.server.routes[method]
			let raw = aero.server.raw[method]
	...
	time(phase) {
		aero.benchmarks[phase] = new Date()
	}

	timeEnd(phase) {
		aero.benchmarks[phase] = new Date() - aero.benchmarks[phase]
	}
	...
	use() {
		for (let i = 0; i < arguments.length; i++) {
			if(aero.server)
				...
				aero.pastMiddleware.push(arguments[i])
		}
	}

	on(eventName, func) {
		aero.pastEvents.push({
			eventName,
			func
		})
	...
	loading(type, what) {
		if(aero.overwriteLoadingMessages)
	...
	loadPage(pageId) {
		...
		if(aero.verbose)
			aero.loading('page', chalk.blue(pageId))
	...
	registerEventListeners() {
		...
		let recompileStyles = function() {
			aero.separator(chalk.magenta)

			let asyncStyleCompileTasks = aero.config.styles.map(function(styleId) {
				if(aero.verbose)
					aero.loading('style', chalk.magenta(styleId))

				let stylePath = path.join(aero.config.path.styles, styleId + '.styl')
				return loadStyle(stylePath).then(function(code) {
					let style = {
						id: styleId,
						code
					}

					aero.events.emit('style loaded', style)
					return style
				})
			})
			...
			return Promise.all(asyncStyleCompileTasks).then(function(results) {
				aero.css = results
				aero.events.emit('all styles loaded')
			})
		}

		// Load all scripts
		let recompileScripts = function() {
			aero.separator(chalk.cyan)

			let asyncScriptCompileTasks = aero.config.scripts.map(function(scriptId) {
				if(aero.verbose)
					aero.loading('script', chalk.cyan(scriptId))

				return loadScript(path.join(aero.config.path.scripts, scriptId + '.js')).then(function(code) {
					let script = {
						id: scriptId,
						code
					}

					aero.events.emit('script loaded', script)
					return script
				})
			})
			...
			return Promise.all(asyncScriptCompileTasks).then(function(results) {
				aero.js = results
				aero.events.emit('all scripts loaded')
			})
		}
		...
		let reloadPages = function() {
			aero.separator(chalk.blue)

			return findPages(aero.config.path.pages)
		...
		aero.events.on('config loaded', Promise.coroutine(function*() {
			...
			// Live reload
			if(!aero.production)
				aero.liveReload = new LiveReload(aero.security, aero.config.ports.liveReload)
			...
			if(aero.config.fonts.length > 0) {
				if(aero.verbose)
					aero.loading('fonts', chalk.bold(aero.config.fonts.join(', ')))

				aero.time('Fonts')

				aero.downloadFonts =
					download('https://fonts.googleapis.com/css?family=' + aero.config.fonts.join('|'))
					.then(style => {
						aero.timeEnd('Fonts')
						return style
					})
			...
			// Load plugins
			aero.separator(chalk.yellow)
			aero.loadPlugins = Promise.each(Object.keys(aero.config.plugins), function(pluginName) {
				if(aero.verbose)
					aero.loading('plugin', chalk.yellow(pluginName))
				...
				if(plugin.init) {
					tasks.push(
						plugin.init(aero, aero.config.plugins[pluginName])
					)
				}
				...
				aero.pluginScripts = aero.pluginScripts.map(code => code.trim())
			})

			// Wait for plugins to finish loading
			yield aero.loadPlugins.then(() => aero.timeEnd('Plugins'))
			...
			aero.layout = yield new Layout(aero.config.path.layout, function(layout) {
				...
				Promise.all([
						recompileStyles().then(() => aero.timeEnd('Styles')),
						recompileScripts().then(() => aero.timeEnd('Scripts'))
					])
					.then(() => aero.time('Pages'))
					.then(Promise.coroutine(function*() {
						if(!aero.downloadFonts)
							return

						aero.fontDefinitions = yield aero.downloadFonts

						// Remove newline characters
						aero.fontDefinitions = aero.fontDefinitions.replace(/(?:\r\n|\r|\n)/g, '')
					}))
					.then(reloadPages)
					.then(() => aero.timeEnd('Pages'))
					.then(function() {
						if(!aero.verbose || !aero.showStyleSize || aero.css.length === 0)
							return

						aero.separator(chalk.magenta)

						aero.css.forEach(function(style) {
						...
						let size = (aero.css.map(style => style.code.length).reduce((a, b) => a + b) / 1024).toFixed(1)
					...
					.then(() => aero.events.emit('all pages loaded'))
			})
		...
		aero.events.on('server started', function() {
			aero.pendingRouteChecks.map(check => check())
		})
		...
		aero.events.on('page loaded', function(page) {
			...
			if(page.controller) {
				page.httpVerbs.forEach(function(method) {
					let controllerMethod = page.controller[method].bind(page.controller)

					if(page.template) {
						aero.server.raw[method.toUpperCase()][page.url] = function(request, response) {
							response.render = getPageRenderFunction(page, request, response)
							aero.server.execute(controllerMethod, request, response)
						}
					} else {
						aero.server.raw[method.toUpperCase()][page.url] = function(request, response) {
							aero.server.execute(controllerMethod, request, response)
				...
				aero.server.raw.GET[page.url] = function(request, response) {
			...
			let css = aero.pluginStyles.join(' ') + aero.css.map(style => style.code).join(' ')
			let js = (aero.liveReload ? aero.liveReload.script : '') + aero.pluginScripts.join(';') + aero.js.map(script => script.code).join(';') // '"use strict";' +
			let mobileMetaTag = '<meta name="viewport" content="width=device-width, minimum-scale=1.0, initial-scale=1.0, user-scalable=yes">'

			let renderLayoutTemplate = function(params) {
				// Hacky, but works for now™
				return aero.layout.template(params)
				...
				if(page.template) {
					...
					if(aero.layout.controller) {
							...
							aero[method](page.url, function(request, response) {
								renderLayout(request, function(layoutControllerParams) {
									response.render = function(params) {
										let code = page.wrap(renderPageTemplate(Object.assign({}, request.globals, params, page.json, page.defaultParams)))
						...
						page.httpVerbs.forEach(function(method) {
							...
							aero[method](page.url, function(request, response) {
								response.render = function(params) {
									let code = page.wrap(renderPageTemplate(Object.assign({}, params, page.json, page.defaultParams)))
		...
		aero.events.on('route modified', function(method, url) {
			// Live reload
			if(aero.liveReload)
				aero.liveReload.broadcast(url)

			// Check route
			aero.checkRoute(method, url).then(
				function resolve() {
					// ...
				},
	...
	checkRoute(method, relativePath) {
		...
		if(aero.config.static.indexOf(relativePath) !== -1)
			return Promise.reject(new Error('static route'))

		if(!aero.serverStarted) {
			aero.pendingRouteChecks.push(aero.checkRoute.bind(this, method, relativePath))
			return Promise.reject(new Error('server not started'))
		}

		let url = `${aero.server.protocol}://localhost:${aero.server.port}/${relativePath}`;
		...
		.then(function(response) {
			...
			let checkHTML5Response = function(response) {
