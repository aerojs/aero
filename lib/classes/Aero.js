...
class Aero {
	constructor(root) {
		...
		this.run = Promise.coroutine(function*() {
			...
			this.pastEvents.forEach(registration => {
				this.events.on(registration.eventName, registration.func)
			})

			// Register past middleware
			this.pastMiddleware.forEach(middleware => {
				this.server.modifiers.push(middleware)
			})

			let config = yield getFile(path.join(this.root, 'config.json'), defaultConfig).then(JSON.parse)
	...
	route(method, url, func) {
		if(!this.server || !this.events) {
			console.error(chalk.red(`Ignoring route ${url}`))
			return
		}
	...
	registerEventListeners() {
		...
		this.events.on('config loaded', Promise.coroutine(function*() {
			// Favicon
			loadFavIcon(this.config.favIcon, imageData => {
			...
			this.loadPlugins = Promise.each(Object.keys(this.config.plugins), pluginName => {
				...
				if(plugin.init) {
					tasks.push(
						plugin.init(this, this.config.plugins[pluginName])
					)
				}
