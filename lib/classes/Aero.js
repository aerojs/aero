...
class Aero {
	constructor() {
		...
		aero.run = Promise.coroutine(function*() {
			...
			aero.config = deepAssign(defaultConfig, config)
			...
			aero.layout = yield new Layout(aero.config.path.layout, function(layout) {
				...
				Promise.all([
					...
					.then(function() {
						...
						let size = (aero.css.map(style => style.code ? style.code.length : 0).reduce((a, b) => a + b) / 1024).toFixed(1)
